class native_ingester::monitoring {
  ...
  $action_url = 'https://sites.google.com/a/ft.com/technology/systems/dynamic-semantic-publishing/extra-publishing/native-ingester-run-book'
  ...
  @@nagios_service { "${hostname}_check_http_json_health_1":
    ...
    notes               => "kafka-proxy reachable",
    service_description => "kafka-proxy is not reachable",
  ...
  @@nagios_service { "${hostname}_check_http_json_health_2":
    ...
    action_url          => $action_url,
    notes_url           => $action_url,
    notes               => "nativerw reachable",
    service_description => "nativerw is unreachable or not healthy.",
    display_name        => "${hostname}_check_http_json",
    tag                 => $content_platform_nagios::client::tags_to_apply,
  }
